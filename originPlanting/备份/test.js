...
function findItemByConditions() {
    ...
    for (let i = 0; i < totalSlots; i++) {
        ...
        if (item) {
            ...
            // 检查是否符合作物条件和 Jacko 价格
            for (const cropKey in cropData) {
                if (itemName === cropData[cropKey].name && stackSize === cropData[cropKey].sellForJackoPrice) {
                    matchedSlots.push(i);
                }
            }

            // 检查是否包含 "Golden" (保留 test.js 中的逻辑)
...
/**
 * 获取补偿后的游戏内时间
 * @param {number} hourOffset - 小时偏移量（可为负数）
 * @param {number} minuteOffset - 分钟偏移量（可为负数）
 * @returns {string} - 格式化后的时间字符串（HH:mm）
 */
function getCompensatedTime(hourOffset = 0, minuteOffset = 0) {
    // 输入合法性检查
    if (typeof hourOffset !== "number" || typeof minuteOffset !== "number") {
        throw new Error("hourOffset 和 minuteOffset 必须是数字");
    }

    // 获取自世界创建以来的总刻数
    let totalTicks = World.getTime();

    // 计算当天的时间（以刻为单位）
    let dayTicks = totalTicks % 24000;

    // 将刻转换为小时和分钟
    let hours = Math.floor((dayTicks / 1000) % 24); // 游戏内一天24小时，+6 是为了调整为从早上6点开始
    let minutes = Math.floor((dayTicks % 1000) * 60 / 1000);

    // 应用误差补偿
    hours += hourOffset; // 加上小时偏移量
    minutes += minuteOffset; // 加上分钟偏移量
    ...
    // 处理小时溢出
    hours = ((hours % 24) + 24) % 24; // 确保小时数在 0 到 23 的范围内
...
function moveToJackoFunc(pos1, pos2) {
    moveToBlock(pos1[0], pos1[1], pos2[0], 1); // Fixed typo pos2[0] instead of pos1[2] and pos2[2] instead of pos2[1]
    Client.waitTick(6);
    ...
    try {
        ...
        if (!checkCropQuantities(64 * 3)) {
            if (!replenishCropBaskets()) {
                isJackoMode = false;
                Client.waitTick(66); // 返回领域
                Chat.say("/realm tp 7OCbefore") // 返回领域
                return false;
            }
        }
        ...
        isJackoMode = false;
        Client.waitTick(66); // 返回领域  <-- 添加返回领域代码
        Chat.say("/realm tp 7OCbefore") // 返回领域  <-- 添加返回领域代码
...
/**
    =========== 定时任务 -  动态时间检查 ==============
 */
let nextJackoSellTime = 0; // 下次 Jacko 售卖的时间戳 (毫秒)
let isFirstRun = true; // 标记是否为首次运行

// function calculateNextJackoSellTime() {
//     const now = Date.now();
//     const currentCompensatedTime = getCompensatedTime(1, 14);
//     let [hours, minutes] = currentCompensatedTime.split(':').map(Number);

//     let targetHour = 7; // 目标时间 7:00 AM
//     let targetDate = new Date(); // 默认为今天

//     if (hours >= targetHour) {
//         // 如果当前时间已经过了 7:00 AM，则设置为明天 7:00 AM
//         targetDate.setDate(targetDate.getDate() + 1); // 设置为明天
//     }

//     targetDate.setHours(targetHour, 0, 0, 0); // 设置为目标小时 7:00:00:000
//     return targetDate.getTime(); // 返回目标时间的毫秒时间戳
// }
...
function jackoSchedule() {
    ...
    if (isFirstRun || now >= nextJackoSellTime) {
        isFirstRun = false; // 标记为非首次运行

        const currentDayTimeTicks = World.getTime() % 24000;
        let waitTicks;

        if (currentDayTimeTicks < 6000) { // 6000 ticks is 7:00 AM (approximately, actually 6am is 0 tick, 7am is 1000 tick, let's use 6000 for 7am for margin)
            waitTicks = 6000 - currentDayTimeTicks;
            Chat.log(`§e当前时间小于 7:00，将在 ${(waitTicks / 20 / 60).toFixed(1)} 分钟后 (游戏内7:00) 执行 Jacko 售卖...`);
        } else {
            waitTicks = 24000 - currentDayTimeTicks + 6000;
            Chat.log(`§e当前时间大于等于 7:00，将在约 ${(waitTicks / 20 / 60).toFixed(1)} 分钟后 (明天游戏内7:00) 执行 Jacko 售卖...`);
        }

        nextTime = waitTicks; // Set nextTime to waitTicks for JsMacros.on('Tick')

        // Delay execution of Jacko sell using Client.waitTick
        Client.waitTick(waitTicks);
        sellToJacko(jackoData); // Execute Jacko sell after waiting

        Chat.log(`§aJacko 售卖计划完成, 下次计划售卖时间为等待 ${waitTicks} ticks.`);

        return nextTime; // Return waitTicks for next schedule
    }
}


// 初始执行定时任务
let nextTime = jackoSchedule();

// 定时检查和重新调度 (例如每分钟检查一次，确保即使脚本运行很久也能重新触发)
JsMacros.on('Tick', JavaWrapper.methodToJava(() => {
    if (World.getTime() % nextTime === 0) {
        nextTime = jackoSchedule();
    }
}));


Chat.log('§aJacko 定时调度已启动.');