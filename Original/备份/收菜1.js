        ...
        while ((this.xStep > 0 && this.currentX <= this.endPos.x) || 
            ...
            for (let z = zStart; 
                ...
                for (let localX = this.currentX;
                     ...
                     (this.xStep < 0 && localX > this.currentX + this.stepSize * this.xStep && localX >= this.endPos.x);
                     localX += this.xStep) {
                    yield Point3D(localX, this.startPos.y, z);
                }
            }
...
// 优化snakeWalk函数，使用FarmAreaManager来管理遍历
function snakeWalk(startPos, endPos, chestPos, itemsToTransfer, playerName) {
    const farmManager = new FarmAreaManager(startPos, endPos);
    
    for (const pos of farmManager.getNextPositions()) {
        moveToBlock(pos);
